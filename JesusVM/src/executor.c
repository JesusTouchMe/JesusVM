...
static void CallInst(Constant* constant) {
	Function* function;
...
static void LoadConstant(Constant* constant) {
...
static void RunWideInstruction() {
	u8 instruction = *vm.ip++;

	switch (instruction) {
		inst(OP_LOAD, index16) {
			op16(index16);
		...
		inst(OP_NEW, const32) {
			op32(const32);
			Constant* constant = GetConstant(vm.currentModule, (u32) const32);
		...
		inst(OP_JMPREL, rel32) {
			op32(rel32);
			JumpUnconditional(rel32);
			break;
		}

		inst(OP_JEQREL, rel32) {
			op32(rel32);
		...
		inst(OP_CALL, const32) {
			op32(const32);
			Constant* constant = GetConstant(vm.currentModule, const32);
...
static void RunInstruction() {
	...
	switch (instruction) {
		...
		inst(OP_NEW, const16) {
			op16(const16);
			Constant* constant = GetConstant(vm.currentModule, (u32) const16);
		...
		default: {
			printf("VM error: unknown opcode %x\n", instruction);
			ExitVM();
			exit(1);
		}
	}
